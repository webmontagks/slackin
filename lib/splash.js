    ...
    dom('p.status',
      active
        ? [
					...
					dom('b.active', active), ' von ',
          dom('b.total', total), ' registrierten Webenthusiasten online.'
        ]
        : [dom('b.total', total), ' Webenthusiasten sind bis jetzt registriert.']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
      ...
      dom('button.loading', 'Einladung erhalten')
    ),
    dom('p.signin',
      'oder ',
      dom(`a href=https://${org}.slack.com target=_top`, 'einloggen'),
      '.'
    ),
    !iframe && dom('footer',
      'powered by ',
